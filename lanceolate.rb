...
# for downloads, its  <<< curl -f -J -O http://0.0.0.0:8080/files/shard/hash >>>
# foe deleting, its <<< curl -X DELETE http://0.0.0.0:8080/file/shard/hash >>>
...
<hr>
  ...
  #{db.execute("SELECT original_name, hash, file_size, uploaded_time, path FROM lanceolate ORDER BY uploaded_time DESC").map { |row|
    shard = row[1][0,2]
    "<tr><td>#{row[0]}</td><td><hr>#{row[1]}</td><td><hr>#{row[2]}</td><td><hr>#{row[3]}</td><td><a href='/files/#{shard}/#{row[1]}'><hr><button type='button'>download</button></a> <button type='button' onclick='if(confirm(\"are you sure you want to delete #{row[0]}?\")) { fetch(\"/files/#{shard}/#{row[1]}\", {method: \"DELETE\"}).then(() => location.reload()); }'>delete</button></td></tr><br>" }.join}
</table>
    <hr>
    <small>#{db.execute("SELECT count(*) FROM lanceolate").first[0]} files in lanceolate</small>
    </body>
    </html>
  HTML
...
post '/upload' do
  ...
  db.execute <<-SQL, [final_path, digest, uploaded_time, file_size, original_name]
  INSERT INTO lanceolate (path, hash, uploaded_time, file_size, original_name) 
  VALUES (?, ?, ?, ?, ?)
